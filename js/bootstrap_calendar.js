...
$.fn.calendar = function(options) {
    ...
    this.each(function(){
        ...
        function showCalendar(){
            ...
            thisMonth = months[month] + ' ' + year;
            showDaysOfMonth(month, year);
                
                
            //next/previous month controls
            var btnNextMonth = $('<td><i class="fa fa-chevron-right"></i></td>');
            ...
            var btnPrevMonth = $('<td><i class="fa fa-chevron-left"></i></td>');
        ...
        function markEvents(month, year){
            ...
            for(var i=0; i< events.length; i++) {
                ...
                if ( events[i][0].split('/')[1] == t_month && events[i][0].split('/')[2] == year ){
                    ...
                    if (events[i][8] === "today") {
                        $('#' + calendar_id + '_' + events[i][0].replace(/\//g, "_") ).addClass('today-event');
                    }
            ...
            $('.manual_popover').click( function(e){
                var dateText = $(e.target).text() + " " + thisMonth;
                document.getElementById("dateDisplay").innerHTML = dateText;

                document.getElementById("eventList").innerHTML = "";

                for(var i=0; i< events.length; i++) {
                    if (events[i][5] === dateText) {
                        var theDiv = document.getElementById("eventList");

                        var contentStr = '<li><div class="timeline-event__icon"><div class="timeline-info"><div class="timeline__timeline-date"><div class="timeline-date--time">' +
                                       events[i][4] +
                                       '</div></div></div></div>' +
                                       '<div class="timeline-event"><h4 class="event-title">' +
                                       events[i][1] + 
                                       '</h4><p>' +
                                       events[i][6] +
                                       '</p>' + 
